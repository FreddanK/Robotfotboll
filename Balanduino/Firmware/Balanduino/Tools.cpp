...
void Tools::printValues() {
  ...
  if(bluetooth){
    if (bluetooth->SerialBT.connected && bluetoothData)
      out = dynamic_cast<Print *> (&bluetooth->SerialBT); // Print using the Bluetooth SPP interface
    else
...
void Tools::setValues(char *input) {
  if (input[0] == 'A' && input[1] == ';') { // Abort
    ...
    if(bluetooth) {
      while (Serial.read() != 'C' && bluetooth->SerialBT.read() != 'C') // Wait until continue is sent
        bluetooth->Usb.Task();
    }
    else {
      while (Serial.read() != 'C');
